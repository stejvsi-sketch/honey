...
const getCachedHomeMemories = unstable_cache(
  async (limit: number) => {
    ...
    const { data, error } = await supabase
      .from('memories').select('id, name, message, color_id, created_at, slug, pinned_until, from_name, wish_reply')
...
const getCachedArchiveMemories = unstable_cache(
  async (page: number, limit: number) => {
    ...
    const [{ data, error }, { count, error: countError }] = await Promise.all([
      supabase.from('memories').select('id, name, message, color_id, created_at, slug, pinned_until, from_name, wish_reply')
...
const getCachedMemoryById = unstable_cache(
  async (id: string) => {
    ...
    const { data, error } = await supabase.from('memories')
      .select('id, name, message, color_id, created_at, slug, pinned_until, from_name, wish_reply').eq('id', id).single();
...
const getCachedMemoriesByCollection = unstable_cache(
  async (themeSlug: string, page: number, limit: number) => {
    ...
    let baseQuery = supabase.from('memories').select('id, name, message, color_id, created_at, slug, pinned_until, from_name, wish_reply');
